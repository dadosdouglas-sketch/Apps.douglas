...
const Kit3EixoRow: React.FC<Kit3EixoRowProps> = ({ 
  ...
  return (
    <tr className={`transition-colors border-b last:border-0 group ${isDarkMode ? 'hover:bg-slate-800/50 border-slate-800' : 'hover:bg-slate-50 border-slate-100'}`}>
      ...
      <td className="py-4 px-3 whitespace-nowrap align-middle">
        <span className={`font-semibold text-xs ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>{item.codFreiocar}</span>
      </td>

      {/* Coluna 4: Descrição (Ou Configuração se Descrição for vazia) */}
      <td className="py-4 px-3 align-middle">
        <span className={`text-xs font-medium block leading-snug uppercase ${isDarkMode ? 'text-slate-400' : 'text-slate-700'}`}>
      ...
      {/* Coluna 6: QTD */}
      <td className="py-4 px-3 text-right pr-4 align-middle w-24">
        {hasPrice && (
